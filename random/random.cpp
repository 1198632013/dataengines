...
void Random::ProcessingDatas(char *data, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        QList<QString> name_and_datas = frame_str.split(FrameHeaderSeparator, QString::SkipEmptyParts);
        if (name_and_datas.size() >= 2) {
            ...
            QList<QString> datas = name_and_datas[name_and_datas.size()-1].trimmed().split(DataSeparator, QString::SkipEmptyParts);
            if (name == "image") {
                // 图片前导帧
                if (datas.length() != 5) {
                    // 图片前导帧异常
            ...
            QList<QString> datas = frame_str.trimmed().split(DataSeparator, QString::SkipEmptyParts);
